...
public class Solution {
   public static ArrayList<Integer> findPath(BinaryTreeNode<Integer> root, int data){
	...
	if(data<root.data) {
		ArrayList<Integer> list1= findPath(root.left, data);
		if(list1==null) {
			return null;
		}else {
			list1.add(root.data);
		...
		ArrayList<Integer> list2= findPath(root.right, data);
		if(list2==null) {
			return null;
		}else {
			list2.add(root.data);
...
public static void main(String[] args) {
	...
	int k= scan.nextInt();
	ArrayList<Integer> ans= findPath(root, k);
	if(ans==null) {
